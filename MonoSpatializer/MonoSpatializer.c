...
#define PLUGIN_UI_URI "urn:bill:MonoSpatializer_ui"
...
void plugin_set_window_size(int *w,int *h,const char * plugin_uri) {
    ...
    (*h) = 275; //set initial height of main window
}
...
void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri) {
    ...
    ui->widget[0] = add_lv2_knob (ui->widget[0], ui->win, 3, "DELAY MILLISECONDS", ui, 25,  12, 171, 226);
    set_adjustment(ui->widget[0]->adj, 20.0, 20.0, 1.0, 40.0, 1.0, CL_CONTINUOS);

    ui->widget[1] = add_lv2_knob (ui->widget[1], ui->win, 4, "IMAGE VOLUME", ui, 218,  98, 113, 80);
    set_adjustment(ui->widget[1]->adj, 0.0, 0.0, -96.0, 0.0, 0.1, CL_CONTINUOS);

    ui->widget[2] = add_lv2_knob (ui->widget[2], ui->win, 5, "LOW PASS FILTER FREQ", ui, 333,  161, 130, 80);
    set_adjustment(ui->widget[2]->adj, 8000.0, 8000.0, 20.0, 20000.0, 100.0, CL_CONTINUOS);

    ui->widget[3] = add_lv2_hslider (ui->widget[3], ui->win, 6, "IMAGE PAN", ui, 338,  100, 120, 48);
    set_adjustment(ui->widget[3]->adj, 20.0, 20.0, -90.0, 90.0, 1.0, CL_CONTINUOS);

    ui->widget[4] = add_lv2_hslider (ui->widget[4], ui->win, 7, "ORIGINAL PAN", ui, 268,  22, 222, 59);
